...
-- ===========================================
-- Strategy 3: Pair Explosion (Rule-Based)
-- ===========================================
AutoPlayStrategies.PairExplosion = {
    name = "PairExplosion",
    
    selectCardsForCrib = function(self, hand)
        if not hand or #hand < 2 then return {} end
        
        -- Look for duplicate ranks to put in crib
        local rank_counts = {}
        for i, card in ipairs(hand) do
            local r = card.rank
            rank_counts[r] = rank_counts[r] or {}
            table.insert(rank_counts[r], i)
        end
        
        -- Find a pair
        for rank, indices in pairs(rank_counts) do
            if #indices >= 2 then
    ...
    selectShopItem = function(self, shopItems, gold)
        ...
        -- Prefer pair-boosting items
        for i, item in ipairs(shopItems) do
            if item.cost and item.cost <= gold then
                local name = string.lower(item.name or "")
                if string.match(name, "pair") then
                    return i
                end
            end
        end
...
function AutoPlayStrategies:listStrategies()
    return {"Random", "FifteenEngine", "PairExplosion"}
end
